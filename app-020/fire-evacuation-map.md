...
## 8. 关键算法
1. **走道栅格图 + Dijkstra**（`src/lib/graph.ts`）：把可行走多边形按 **0.25m** 栅格化，可行走判定为「射线法严格内部 **∪ 多边形边界**（0.5mm 容差的点到边段判定）」——共边多边形共享同一条边界栅格行，两侧内部栅格点与边界行正交相邻而自然连通；**不做膨胀**（膨胀会把拐弯墙体内侧填成可行走造成切角、把 ≤0.5m 的真缝隙桥成虚连、让仅角点接触的两段对角斜穿）。安全出口按 **2.5m**、门按 **1.5m** 吸附到最近栅格点，且接入直连边必须**全程落在可行走栅格上**（半格间距采样圆整判定，穿墙即拒绝，出口/门因此可能被正确判为未连通）；附加节点只与挂载栅格点**双向连这一条边**（不再向落点周围整圈连直线边，也不再把出口回连半径硬编码成门的 1.5m）。8 邻连通，对角要求两个正交邻居都可行（防切角穿墙）；二叉堆多源 Dijkstra 得每个栅格点到最近出口的路径距离。栅格数超过 8,000,000 直接抛 `floor too large for grid`。
2. **房间疏散距离**（`engine.ts` 的 `roomWorstTravelM`）：房间内采样 0.5m 栅格点 **加上全部多边形顶点**（保证非凸房间的最远角、以及窄于/小于一格而没有内部栅格点的小房间不漏测），房间内取「最远点 → 房间门」直线段，再加上门到出口的路径距离；房间内本身有出口时只算房内直线。门推断出来但所有门都接不上走道时报 `DOOR_NOT_CONNECTED`（error），不再静默当成合格。
...
## 10. 验收标准
- 单元测试 **7 个文件 / 64 个用例**全部通过（vitest 2.1.9，`npm test`）：疏散距离 25 组、灭火器覆盖 10 组、检查台账 7 组、编号 6 组、store 回归 10 组、规则切换 4 组、性能 2 组。
- 疏散距离：25 组沿路径用例与手工沿路径测量的误差 < 0.5m；其中第 04 组必须证明「直线距离 ≤40m 看着合格、沿路径 >50m 实际超标」被判 `TRAVEL_EXCEED` 且 `pass=false`；第 21–25 组为真实路径回归：出口经非走道空间不得假连通、走道间真缝隙不得虚桥、小房间顶点必须参与房内测距、双拐弯不切角、门/出口接入边不得穿墙。
